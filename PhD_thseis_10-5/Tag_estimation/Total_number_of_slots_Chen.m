function [T_total_avg] = Total_number_of_slots_Chen(no_itera,Number_of_tags,alpha)
% clear;
% clc;
% Number_of_tags= 11;
% no_itera=2;
...
for j=1:no_itera
    ...
    while (rem_tags)>0
        ...
        estim_rem_tags =chen_estimation(Ne,Nc,Ns,L);
        rem_tags=rem_tags-Ns;                                                                                    % number of remainig tags
        read_tags= read_tags+Ns;
        T_total(j)= round(T_total(j)+0.8*Ns+Nc+Ne);                                  % the actual time need for all tag to be read in each iteration.
